...
3. **What happens with degenerate stored-value distributions?** If
   only one value is ever stored (so all stored patterns are identical),
   the simplest ribbon solution is the trivial all-zero one, and
   non-members all decode to whatever value pattern 0 belongs to. This
   is testable and worth quantifying as a corner case for the cipher-map
   security argument.
